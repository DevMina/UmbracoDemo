(function () {
    ...
    angular
        .module("umbraco")
        .component("umbCustomMediaPicker3PropertyEditor", {
    ...
    function MediaPicker3Controller($scope, $element, editorService, clipboardService, localizationService, overlayService, userService, entityResource, $attrs, umbRequestHelper, $injector, uploadTracker, editorState) {
        ...
        vm.labels = {};
        vm.ContentTypesWithRelatedMedia = ["newsItem", "event", "photoAlbum", "videoAlbum", "contentTemplateDetailsPage"];
        ...
        function showMediaUpload(show) {
            if (show) {
                document.body.classList.remove("no-media-upload");
            }
            else {
                document.body.classList.add("no-media-upload");
            }
        }
        ...
        function addMediaAt(createIndex, $event) {
            ...
            if (vm.node && vm.node.contentTypeAlias && vm.ContentTypesWithRelatedMedia.includes(vm.node.contentTypeAlias)) {
